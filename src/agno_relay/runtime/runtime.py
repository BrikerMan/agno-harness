"""AguiRuntime — the batteries-included assembly.
...
:class:`~agno_relay.runtime.scope.RunScope` and may set ``user_id``,
...
:class:`~agno_relay.runtime.translator.EventTranslator` yourself, call
...
class AguiRuntime:
    """Drives an Agno agent and emits a well-formed AG-UI event stream.
    ...
    stores:
        Optional :class:`~agno_relay.stores.Stores` for records Agno
    ...
    trace_dir:
        ...
        ``$BETTER_AGNO_TRACE_DIR``, and to off when that is unset. See
        :mod:`agno_relay.runtime.tracing`.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.runner = AgentRunner(agent, enable_subagent_streaming=enable_subagent_streaming)
    ...
    def register_module(self, module: BridgeModule) -> AguiRuntime:
    ...
    def register_parser(self, event_type: Any, parser: EventParser) -> AguiRuntime:
    ...
    def register_tool_filter(self, tool_filter: ToolFilter) -> AguiRuntime:
        """Append a filter to the chain (``None`` drops, an event keeps)."""
        self.tool_filters.add(tool_filter)
        return self

    def on_pre_run(self, hook: PreRunHook) -> AguiRuntime:
    ...
    def on_post_run(self, hook: PostRunHook) -> AguiRuntime:
    ...
    async def stream(
        ...
        *,
        ...
        async for event in self.stream_events(run_input, request=request, user_id=user_id):
    ...
    async def stream_events(
        ...
        *agent*, not to the wire, and opening the run first is what keeps a
        ...
        """
        scope = self._make_scope(run_input, user_id)
    ...
    def _make_scope(self, run_input: RunAgentInput, user_id: str | None) -> RunScope:
