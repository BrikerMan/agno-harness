...
class ObservabilityModule(Module):
    ...
    ----------
    agent_name:
        Used to build the default span name ``{agent}.turn-run``. Defaults to
        ``"agent.turn-run"``; :meth:`bind_agent` takes ``agent.name`` instead
        (so ``movie-assistant`` becomes ``movie-assistant.turn-run``).
    ...
    def bind_agent(self, agent: Any) -> ObservabilityModule:
        ...
        agent and three specialists otherwise produces three traces all called
        ``agent.turn-run``, and telling them apart means opening each one.
        """
    ...
    async def on_run_start(self, run: RunScope) -> AsyncIterator[BaseEvent]:
        ...
        # Inject multi-dimensional attributes for Langfuse / OTel dashboard filtering
        meta = dict(getattr(run, "metadata", {}) or {})
        if isinstance(run.forwarded_props, dict):
            for k in ("tags", "platform", "environment", "env"):
                if k in run.forwarded_props:
                    meta.setdefault(k, run.forwarded_props[k])

        for k, v in meta.items():
            if v is not None:
                if k == "tags" and isinstance(v, (list, tuple)):
                    attributes["tags"] = json.dumps(list(v))
                    attributes["langfuse.tags"] = ",".join(str(t) for t in v)
                else:
                    attributes[k] = str(v)
    ...
    def _name_for(self, run: RunScope) -> str:
        ...
        return turn_run_span_name(self._agent_name)


def turn_run_span_name(agent_name: str | None) -> str:
    """Default root span: ``<agent-name>.turn-run``.

    Unnamed agents become ``agent.turn-run``. The suffix is not doubled if the
    name already ends with it.
    """
    parts = (agent_name or "").strip().split()
    slug = "-".join(parts).lower() if parts else DEFAULT_AGENT_SLUG
    if slug.endswith(TURN_RUN_SUFFIX):
        return slug
    return f"{slug}{TURN_RUN_SUFFIX}"
...
__all__ = ["ObservabilityModule", "turn_run_span_name"]
