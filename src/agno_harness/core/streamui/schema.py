...
class BlockSchema(BaseModel):
    ...
    @classmethod
    async def handle_action(cls, action: str, payload: Mapping[str, Any], event: Any) -> Any | None:
        """Class-First interactive card action handler.

        When a user clicks a button on this card in Teams or Lark, the action is routed
        directly to this class method. Subclasses can override this to implement
        self-contained button callbacks without global registries or circular imports.
        """
        return None
...
class ItemSchema(BaseModel):
    ...
    @classmethod
    async def handle_action(cls, action: str, payload: Mapping[str, Any], event: Any) -> Any | None:
        """Class-First interactive item action handler."""
        return None
...
class CardCatalog:
    ...
    async def dispatch_action(
        ...
        """Route an interactive action to the matching Class-First card schema.

        Resolution strategy:
        1. If payload specifies "schema" or "schema_name", route directly to that schema.
        2. If action_id is namespaced as "schema_name:action_verb", route to that schema.
        3. Fallback: try all registered schemas until one returns a non-None result.
        """
        schema_name = payload.get("schema") or payload.get("schema_name")
        sub_action = action_id

        if not schema_name and ":" in action_id:
            prefix, rest = action_id.split(":", 1)
            if prefix in self._blocks or prefix in self._items:
                schema_name = prefix
                sub_action = rest

        if schema_name:
            target_cls = self._blocks.get(str(schema_name)) or self._items.get(str(schema_name))
            if target_cls is not None and hasattr(target_cls, "handle_action"):
                result = await target_cls.handle_action(sub_action, payload, event)
                if result is not None:
                    return result

        # Fallback: scan registered schemas
        for cls_target in (*self._blocks.values(), *self._items.values()):
            if hasattr(cls_target, "handle_action"):
                result = await cls_target.handle_action(sub_action, payload, event)
                if result is not None:
                    return result
