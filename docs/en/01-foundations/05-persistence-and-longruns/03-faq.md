...
Agno’s `AgentSession` is a **post-run transaction**: this turn’s Q&A hits SQL only after `arun()` finishes. Mid-run `GET /api/v1/threads/{id}/messages` does not contain this prompt.
...
Replay from `GET /api/v1/threads/{id}/frames` alone restores prompt + reasoning + tools + incremental text.
...
Send `POST /api/v1/channels/web/agui?long-run=1`. `LongRunManager` hosts the agent on its own `asyncio.Task` and writes frames to the hot log. On return: `GET /api/v1/threads/{id}/active` still `running` → `GET /api/v1/runs/{id}/attach?after=`. Client sequence: [Web 04](../../03-clients/01-web-react/04-attach-and-longrun.md).
...
```text
Reload
  → GET /api/v1/threads/{id}/frames     already-emitted text and tools
  → GET /api/v1/threads/{id}/active     runId + input; prepend the prompt if missing; isStreaming = true
  → GET /api/v1/runs/{runId}/attach?after=  continue
```
...
### Q5: Mid first run, `GET /api/v1/threads` does not list this session?
