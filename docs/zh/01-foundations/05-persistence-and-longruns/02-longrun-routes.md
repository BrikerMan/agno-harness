...
Relay 路径用 `relay.get_router(...)`，同样把 `long_runs` 编进去。没挂就是 `/api/v1/runs/*` **404**。

| Route | 作用 |
| --- | --- |
| `POST /api/v1/channels/web/agui?long-run=1` | 后台跑；关 tab 不停（亦 `?long_run=1`、旧 `?detach=1`） |
| `GET /api/v1/runs/{id}/attach?after=` | **canonical** 挂回正在输出的 run |
| `GET /api/v1/runs/{id}/stream?after=` | deprecated，与 `/attach` 等价 |
| `GET /api/v1/threads/{id}/active` | 仍在跑 **或 HITL paused**（`is_open`），含 `input`。paused 不再写帧（`is_producing` 为假），attach/tail 吐完已存的就结束 |
| `POST /api/v1/runs/{id}/abort` | 用户点 Stop。响应 `{ aborted: bool }` |
...
| Resume | `GET /api/v1/runs/{id}/attach?after=` | 断线/刷新按游标续写 |
