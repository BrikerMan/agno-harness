...
Agno 的 `AgentSession` 是 **post-run 事务**：整轮 `arun()` 结束才把本轮问答写入 SQL。中途去 `GET /api/v1/threads/{id}/messages`，这一轮的用户句还不在。
...
回放单靠 `GET /api/v1/threads/{id}/frames` 就能还原问句 + 思考 + 工具 + 增量回答。

### Q2: 刷新后 Stream 断了、不再输出？

普通 HTTP 跟 TCP 绑死。刷新 RST 掉连接，若 run 没和连接脱钩，协程退出。

发送带 `POST /api/v1/channels/web/agui?long-run=1`；`LongRunManager` 把 Agent 放到独立 `asyncio.Task`，帧写入热 log。回来：`GET /api/v1/threads/{id}/active` 仍 `running` → `GET /api/v1/runs/{id}/attach?after=`。前端时序见 [Web 04](../../03-clients/01-web-react/04-attach-and-longrun.md)。
...
```text
Reload
  → GET /api/v1/threads/{id}/frames     已吐出的字和工具
  → GET /api/v1/threads/{id}/active     runId + input；缺问句就置顶；isStreaming = true
  → GET /api/v1/runs/{runId}/attach?after=  续写
```
...
### Q5: 首轮进行中刷新，`GET /api/v1/threads` 侧栏找不到？
